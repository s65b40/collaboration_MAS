...
output_file_path = '../../output/fact/G2-P3-I2-C3.json'  # G2-P3-I2-C3
...
def run_predictions_simultaneous_talk(selected_agents):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        future_to_agent = {executor.submit(predict_with_agent, agent): agent for agent in
                           selected_agents}
...
def instructed_prediction_flow(evidence_data, round):
    ...
    for i in range(1, round + 1):
        ...
        # Interaction mode
        current_predictions = run_predictions_one_by_one_talk(selected_agents) # Line8  9

        # predictions = merge_predictions(current_predictions, agents)

        # Decide result
        final_decision, input_tokens, output_tokens = instructor.decide_final_decision(
            current_predictions, agents)
        update_token_counts(input_tokens, output_tokens)
        if final_decision:
            return final_decision, i

        # Update history
        history, input_tokens, output_tokens = instructor.generate_historical_information(
            current_predictions)
        update_token_counts(input_tokens, output_tokens)
...
def generate_medical_predictions(evidence_data_path, output_file_path):
    ...
    for hadm_id, hadm_data in tqdm(list(evidence_data.items()),
                                   desc="Processing records", unit="record"):
