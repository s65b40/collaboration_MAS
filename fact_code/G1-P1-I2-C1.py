...
output_file_path = '../../output/fact/G1-P1-I2-C1.json'  # G1-P1-I2-C1
    ...
    for i in range(1, round + 1):
        ...
        for agent in agents:
            for agent_again in agents:
                agent.add_history_last_round(agent_again.name, predictions[agent_again.name], i)
...
def generate_medical_predictions(evidence_data_path, output_file_path):
    ...
    for hadm_id, hadm_data in tqdm(list(evidence_data.items()), desc="Processing records", unit="record"):
        ...
        prediction, rounds = sequential_prediction_flow_oredred(hadm_data, 10)
